     ...
     <Transition name="modal">
      <div v-if="showModal" class="bg-white rounded-lg p-8 max-w-md w-full mx-4">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-2xl font-semibold">{{ selectedBeds?.title }}</h3>
        ...
        <!-- Product Description -->
        <div class="mb-6">
          <p class="text-gray-600">{{ selectedBeds?.description }}</p>
          <div class="flex items-center gap-2 mt-3">
            <span class="text-2xl font-bold text-primary">{{ selectedBeds?.newPrice }}₽</span>
            <span class="text-lg text-gray-400 line-through">{{ selectedBeds?.oldPrice }}₽</span>
          </div>
        </div>
...
const openInquiryForm = (beds) => {
  selectedBeds.value = beds;
...
.beds-slider .splide__arrow {
  background: rgba(5, 128, 199, 0.1) !important;
}

.beds-slider .splide__arrow svg {
